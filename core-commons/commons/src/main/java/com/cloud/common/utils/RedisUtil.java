...
public class RedisUtil {
    ...
    @Value("${spring.redis.order.database:1}")
    private Integer redisOrderDatabase;
    @Autowired
    private JedisPool jedisPool;
    @Value("${spring.redis.failure.time:86400}")
     ...
     * @return
     */
    public boolean vagueDeleteKey(String pattern){
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(redisDatabase);
    ...
    public String set(String key, String value) {
        ...
        try {
            jedis = jedisPool.getResource();
            jedis.select(redisDatabase);
            return jedis.setex(key, failureTime, value);
        } catch (Exception e) {
    ...
    public String unlimitedSet(String key, String value) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(redisDatabase);
    ...
    public String setAndTime(String key, String value,Integer failureTime) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(redisDatabase);
    ...
    //redis触发命令
    public String setOrderTime(String key, String value,Integer failureTime) {
        Jedis jedis = null;
        try {
            jedis = jedisPool.getResource();
            jedis.select(redisOrderDatabase);
            return jedis.setex(key, failureTime, value);
        } catch (Exception e) {
