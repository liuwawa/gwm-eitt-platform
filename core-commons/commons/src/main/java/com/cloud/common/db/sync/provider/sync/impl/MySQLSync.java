package com.cloud.common.db.sync.provider.sync.impl;
...
import com.cloud.common.db.sync.provider.utils.Tool;
    ...
    public String assembleSQL(String srcSql, Connection conn, JobInfo jobInfo) throws SQLException {
        ...
        while (rs.next()) {
            ...
            for (int index = 0; index < fields.length; index++) {
                sql.append(rs.getString(fields[index]) == null ? "" : "'").append(rs.getString(fields[index]))
                        .append(index == (fields.length - 1) ? (rs.getString(fields[index]) == null ? "" : "'") : (rs.getString(fields[index]) == null ? "," : "',"));
            }
            sql.append("),");
            count++;
        }
