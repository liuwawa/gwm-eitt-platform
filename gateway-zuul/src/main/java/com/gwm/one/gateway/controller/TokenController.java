...
import com.gwm.one.gateway.feign.LogClient;
import com.gwm.one.gateway.feign.Oauth2Client;
import com.gwm.one.gateway.feign.UserClient;
...
import com.gwm.one.model.oauth.SystemClientInfo;
...
import com.gwm.one.utils.ZuulUtils;
...
public class TokenController {
    ...
    private void initTokenCookie(String username, HttpServletRequest request, HttpServletResponse response, Map<String, Object> tokenInfo) {
        if ("10000".equals(String.valueOf(tokenInfo.get("errorCode")))) {
            ...
            redisUtils.delString(UserInterceptor.USER_CODE + username);
            redisUtils.setString(UserInterceptor.USER_CODE + username, val);
    ...
    private void getOut(String username, HttpServletRequest request) {
        ...
        if ("1".equals(request.getParameter("out"))) {
            if (username != null) {
                String tok = redisUtils.getString(UserInterceptor.USER_CODE + username);
                String[] split = tok.split("@");
                if (split != null && split.length == 4) {
                    if (redisUtils.getString(SysConstants.PAST + username) != null) {
                        redisUtils.delString(SysConstants.PAST + username);
                    }
                    redisUtils.setString(SysConstants.PAST + username, split[3]);
